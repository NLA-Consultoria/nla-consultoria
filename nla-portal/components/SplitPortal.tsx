...
// Dica: em Next.js, você pode substituir estes <img> por <Image> de 'next/image'
...
// Paleta (referência)
const NLA_BLUE = '#2A27C6';
const NLA_ACCENT = '#1D4ED8';
...
function useLightSEO() {
  React.useEffect(() => {
    const title = 'NLA Consultoria — Crescer Vendas e Organizar a Empresa';
    const desc = 'Portal da NLA: cresça as vendas via novos canais e organize sua empresa com processos e automações sob medida.';
    ...
    const mDesc = ensureMeta('description', desc);
    const ogTitle = ensureMeta('og:title', title, 'property');
    const ogDesc = ensureMeta('og:description', desc, 'property');
    const ogType = ensureMeta('og:type', 'website', 'property');
    // Placeholder: imagem OG representando o split
    const ogImage = ensureMeta('og:image', 'https://dummyimage.com/1200x630/ffffff/1d4ed8&text=NLA+Portal', 'property');

    return () => {
      // Opcional: restaurar o título anterior
      document.title = prevTitle;
      // Mantém as metas inseridas (normal para SPAs). Remover se necessário.
    };
  }, []);
}
...
export default function SplitPortal() {
  ...
  const subtitleVisibility = SUBTITLE_MOBILE_DEFAULT_VISIBLE
    ? // visível por padrão no mobile, some no hover-off em >= md e aparece no hover/focus
    ...
    : // oculto por padrão; aparece no hover/focus e quando revelado por toque
  ...
  return (
    ...
    >
      ...
      {/* Linha diagonal (pseudo via before:) no centro com leve rotação */}
      <div
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              backdropFilter: 'blur(2px)', // opcional: backdrop-blur-sm
            }}
          />
